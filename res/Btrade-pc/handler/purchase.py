...
class MyPurchaseHandler(BaseHandler):
    ...
    def get(self, page=0):
        ...
        purchases = self.db.query("select p.*,u.nickname,u.name from purchase p left join users u on p.userid = u.id where p.userid = %s order by p.createtime desc limit %s,%s", self.session.get("userid"), page * config.conf['POST_NUM'], config.conf['POST_NUM'])
