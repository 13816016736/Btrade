...
for i in range(nrows):
    ...
    print data[2],data[4],data[-1],data[-2]
    conn = MySQLdb.connect(host='127.0.0.1',user='root',passwd='ycg20160401',db='yaoyy',port=3306, charset="utf8")
    cursor = conn.cursor()
    cursor.execute("select id from category where name = %s", (data[2],))
    variety = cursor.fetchone()
    if variety:
        cursor.execute("update category set spec = %s,unit_desc = %s,alias = %s where id = %s ", (data[-2], data[-1], data[4], variety[0]))
        print
    else:
        print False
    conn.commit()
...
#
#
#     print data[2],data[3],data[-1],data[-2]
