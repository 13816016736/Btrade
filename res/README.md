...
微信号给严飞，让严飞定好头像功能介绍啥的。   郭老融资约定时间没？
...
7.注册环节药材购服务条款
8.注册页面用户类型 个人企业类型 企业类型都显示出来  去掉滑动验证码  涉及移动端注册的用户类型
9.注册真实姓名未保存
10.账户设置修改密码  密码格式验证做服务端验证即可
11.账户设置关注品种bug*
12.去掉都不显示"身份真实"  迭代加上"身份真实"
13.跟@代雷沟通品种数据加上单位*
14.发布采购单  上传图片  x掉后  再上传提示只能传一张图  session中没有删除uploadfile  采购品种质量要求 标签选择的显示在前面  文本域换行  供应商要求没有  自动注册用户名搞简单的
16.发布采购单 价格不填的问题  其他地方不显示价格   解决方案:默认存个负数 比如 -1  其他地方显示判断下 如果小于0 则不显示
17.我的采购 采购单时间  时分秒显示不对
18.我的采购 采购单和采购品种的分享框不同
19.报价结束不能报价
20.已经关闭了 就不要"x"按钮
21.移动端 首页 账户类型显示不对 采购单详情页  交货地显示省市 账户类型也显示不对   登录页面  注册按钮移下来  移动端上传图片 点哪种类型图片  上传页面就显示哪种类型图片说明   报价成功通知供应商?应该通知采购商  用户类型  名称统一
22.所有图片预览都是固定尺寸显示
